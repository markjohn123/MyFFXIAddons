...
function equip_unleashing_gear(spell, cast_set)
	if not cast_set then
		cast_set = {}
	end
	if buffactive['unleash'] 
		 and spell
		 and spell.prefix == '/pet'
	then
	-- If unleash is on, keep on unleash gear
		if spell and spell.en and sets.PET and sets.PET[spell.en] then
			if sets.PET[spell.en].pet then
				precast_set = set_combine(precast_set,build_baseset_sets(nil,sets.PET[spell.en].pet))
		  else 
				precast_set = set_combine(precast_set,build_baseset_sets(nil,sets.PET[spell.en]))
			end
		end
		-- If aftermath is up, don't swap out weapons
		-- But if it isn't, have at it!!!  Do maximum carnage
		-- TODO:
		if not (buffactive['relicaftermath'] 
		   or buffactive['mythicaftermath'])
		then
			-- Make sure you have your weapons on
			precast_set = set_combine(midcast_set, sets.weapons[weapon])
		end
		equip(precast_set)
		return
end
...
function precast(spell)
	...
	-- If unleash is on, keep on unleash gear
		...
		equip(precast_set)
		return
	elseif (not T{"Maltha", "Maltana"}:contains(player.name)) and spell.en ~= "Deploy" and (midaction() or pet_midaction()) then
...
function pet_midcast(spell)
	...
	if spell and player.main_job == "PUP" then
...
function aftercast(spell)
	-- Kludge -- Don't execute aftercast is monster is doing a tp move
...
--		  print_set(finalset)
			...
			-- If we're using idle_in, we're going to loose our tp anyway.  Equip the Kerehcatl for attack/acc
			if idle_in and activity == "midcast" and T{"NIN", "DNC"}:contains(player.main_job) then
				if sets.PET[spell.en] and sets.PET[spell.en].sub then
					...
					finalset = set_combine(finalset, T{sub="Kerehcatl"})
				end
			end
		end
		...
		if cmd == 'weapon' and table.length(args) > 1 then
			...
			if table.containskey(sets.weapons, weapon_name) then
				...
				if string.contains(string.lower(weapon), "guttler") 
					 ...
					 or string.contains(string.lower(weapon), "farha") then
					if string.contains(string.lower(weapon), "guttler") then
						send_command("bh amws Onslaught")
						send_command("bh dws Onslaught")
					elseif string.contains(string.lower(weapon), "aymur") then
						send_command('bh amws "Primal Rend"')
						send_command("bh dws Calamity")
					elseif string.contains(string.lower(weapon), "farsha") then
						send_command("bh amws Cloudsplitter")
						send_command("bh dws Cloudsplitter")
					end
					
					if T{"NIN", "DNC"}:contains(player.sub_job) then
						send_command("bh ready 10")
					else
						send_command("bh ready 15")
					end
				end
			else
...
function equip_zone_gear()
	if zone_gear and zone_gear[world.area] then
		equip(zone_gear[world.area])
...
zone_gear['Western Adoulin'] = {
	body = "Councilor's Garb"
}
zone_gear['Eastern Adoulin'] = {
	body = "Councilor's Garb"
}
